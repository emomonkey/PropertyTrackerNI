...
CREATE OR REPLACE VIEW ab_historic_cnty_views as
  SELECT  cntyvol.COUNTY, cntyvol.YEAR, cntyvol.MONTH, cntyvol.VOLPC, soldvol.prcsold, prcchg.percent_price, row_number() OVER ()   id
  FROM
    (SELECT vcnty.county, vcnty.year, vcnty.month, ROUND((vcnty.volsum / tvol.volsum) * 100,2) VOLPC
     FROM
       (SELECT
          COUNTY,
          YEAR,
    ...
    (SELECT volv.county, volv.year, volv.month, ROUND( (soldval /volsum * 100) ,2) prcsold
     ...
     WHERE soldv.county = volv.county AND soldv.year = volv.year AND soldv.month = volv.month
    ) soldvol,
    (SELECT COUNTY, YEAR, MONTH, PERCENT_PRICE * 100 PERCENT_PRICE
     FROM (
            SELECT
              COUNTY,
              YEAR,
              ...
              OVER (PARTITION BY county
                ORDER BY year, month)) * 100, 2), 0) PERCENT_PRICE,
              valtime
            FROM (
                   SELECT
                     COUNTY,
