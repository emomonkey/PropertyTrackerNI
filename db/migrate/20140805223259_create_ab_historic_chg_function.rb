...
SELECT ps.vmonth, ps.vyear, stype, ps.spid , ps.price  - allvals.avgprice,   ps.beds ,'Semi-detached' ,   ps.price  - allvals.avgprice, now()
FROM (SELECT  'Semi-detached' as propertytype, beds, AVG(psv.price) price,
...
INSERT INTO historic_analyses( month, year, search_types_id, search_params_id, resulttext, beds, propertytype, resultvalue, created_at)
SELECT ps.vmonth, ps.vyear, stype, ps.spid , ps.price  - allvals.avgprice,   ps.beds ,'Detached' ,   ps.price  - allvals.avgprice, now()
FROM (SELECT  'Detached' as propertytype, beds, AVG(psv.price) price,
        EXTRACT(MONTH FROM ps.created_at) as vmonth, EXTRACT(YEAR FROM ps.created_at) as vyear, sp.id as spid,
        EXTRACT(MONTH FROM ps.created_at - INTERVAL '1 MONTH') as prvmonth, EXTRACT(YEAR FROM ps.created_at - INTERVAL '1 MONTH') as prvyear
      FROM property_sites ps, property_site_values psv , search_params sp WHERE ps.id = psv.property_site_id
...
INSERT INTO historic_analyses( month, year, search_types_id, search_params_id, resulttext, beds, propertytype, resultvalue, created_at)
SELECT ps.vmonth, ps.vyear, stype, ps.spid , ps.price  - allvals.avgprice,   ps.beds ,'Terrace' ,   ps.price  - allvals.avgprice, now()
FROM (SELECT  'Terrace' as propertytype, beds, AVG(psv.price) price,
        EXTRACT(MONTH FROM ps.created_at) as vmonth, EXTRACT(YEAR FROM ps.created_at) as vyear, sp.id as spid,
        EXTRACT(MONTH FROM ps.created_at - INTERVAL '1 MONTH') as prvmonth, EXTRACT(YEAR FROM ps.created_at - INTERVAL '1 MONTH') as prvyear
      FROM property_sites ps, property_site_values psv , search_params sp WHERE ps.id = psv.property_site_id
...
INSERT INTO historic_analyses( month, year, search_types_id, search_params_id, resulttext, beds, propertytype, resultvalue, created_at)
SELECT ps.vmonth, ps.vyear, stype, ps.spid , ps.price  - allvals.avgprice,   ps.beds ,'Townhouse' ,   ps.price  - allvals.avgprice, now()
FROM (SELECT  'Townhouse' as propertytype, beds, AVG(psv.price) price,
        EXTRACT(MONTH FROM ps.created_at) as vmonth, EXTRACT(YEAR FROM ps.created_at) as vyear, sp.id as spid,
        EXTRACT(MONTH FROM ps.created_at - INTERVAL '1 MONTH') as prvmonth, EXTRACT(YEAR FROM ps.created_at - INTERVAL '1 MONTH') as prvyear
      FROM property_sites ps, property_site_values psv , search_params sp WHERE ps.id = psv.property_site_id
...
INSERT INTO historic_analyses( month, year, search_types_id, search_params_id, resulttext, beds, propertytype, resultvalue, created_at)
SELECT ps.vmonth, ps.vyear, stype, ps.spid , ps.price  - allvals.avgprice,   ps.beds ,'Site' ,   ps.price  - allvals.avgprice, now()
FROM (SELECT  'Site' as propertytype, beds, AVG(psv.price) price,
              EXTRACT(MONTH FROM ps.created_at) as vmonth, EXTRACT(YEAR FROM ps.created_at) as vyear, sp.id as spid,
              EXTRACT(MONTH FROM ps.created_at - INTERVAL '1 MONTH') as prvmonth, EXTRACT(YEAR FROM ps.created_at - INTERVAL '1 MONTH') as prvyear
      FROM property_sites ps, property_site_values psv , search_params sp WHERE ps.id = psv.property_site_id
...
INSERT INTO historic_analyses( month, year, search_types_id, search_params_id, resulttext, beds, propertytype, resultvalue, created_at)
SELECT ps.vmonth, ps.vyear, stype, ps.spid , ps.price  - allvals.avgprice,   ps.beds ,'Cottage' ,   ps.price  - allvals.avgprice, now()
FROM (SELECT  'Cottage' as propertytype, beds, AVG(psv.price) price,
        EXTRACT(MONTH FROM ps.created_at) as vmonth, EXTRACT(YEAR FROM ps.created_at) as vyear, sp.id as spid,
        EXTRACT(MONTH FROM ps.created_at - INTERVAL '1 MONTH') as prvmonth, EXTRACT(YEAR FROM ps.created_at - INTERVAL '1 MONTH') as prvyear
      FROM property_sites ps, property_site_values psv , search_params sp WHERE ps.id = psv.property_site_id
...
INSERT INTO historic_analyses( month, year, search_types_id, search_params_id, resulttext, beds, propertytype, resultvalue, created_at)
SELECT ps.vmonth, ps.vyear, stype, ps.spid , ps.price  - allvals.avgprice,   ps.beds ,'Apartment' ,   ps.price  - allvals.avgprice, now()
FROM (SELECT  'Apartment' as propertytype, beds, AVG(psv.price) price,
        EXTRACT(MONTH FROM ps.created_at) as vmonth, EXTRACT(YEAR FROM ps.created_at) as vyear, sp.id as spid,
        EXTRACT(MONTH FROM ps.created_at - INTERVAL '1 MONTH') as prvmonth, EXTRACT(YEAR FROM ps.created_at - INTERVAL '1 MONTH') as prvyear
      FROM property_sites ps, property_site_values psv , search_params sp WHERE ps.id = psv.property_site_id
