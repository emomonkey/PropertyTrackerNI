class PopulateResults
  ...
  def fndprice
    ...
    sSql = 'SELECT sites.property_site_id, sites.title as title, psvmin.price as first_price, psvmax.price as last_price FROM '\
    '(SELECT property_site_id, title, COUNT(property_site_id) AS cnt FROM property_site_values, property_sites WHERE property_sites.id '\
    '= property_site_values.property_site_id GROUP BY property_site_id, title HAVING cnt > 1) sites, (SELECT property_site_id, MIN(created_at) created_at '\
    'FROM property_site_values GROUP BY property_site_id) min_dates, (SELECT property_site_id, MAX(created_at) created_at FROM property_site_values GROUP BY property_site_id) max_dates,'\
    ' property_site_values psvmin, property_site_values psvmax  WHERE sites.property_site_id = min_dates.property_site_id  AND sites.property_site_id = max_dates.property_site_id AND '\
    ' max_dates.created_at = psvmax.created_at AND max_dates.property_site_id = psvmax.property_site_id AND sites.property_site_id = min_dates.property_site_id AND min_dates.created_at = psvmin.created_at AND min_dates.property_site_id = psvmin.property_site_id'
